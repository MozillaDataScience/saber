...
ipak(c("tidyverse", "DT", "htmltools", "plotly"))
```
...
# Motivation
This is an experiment report for the ``r experiment$experiment_slug`` study.
...
create_plot <- function(metric_summary) {
  plot <- ggplot(metric_summary, aes(x=metric, y=exp, colour=sig)) +
...
```{r}
tagList(lapply(summary_list, create_plot))
```
...
plot_pct_changes_decile <- function(metric_summary) {
  ...
  plot <- ggplot(metric_summary, aes(x=pct_value, y=percentile)) + 
...
```
```{r}
tagList(lapply(summary_list, plot_pct_changes_decile))
```
